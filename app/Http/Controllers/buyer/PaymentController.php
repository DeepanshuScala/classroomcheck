...
class PaymentController extends Controller {
    ...
    public function index(Request $request) {
        ...
        return view('account.payment.account_payment_system', compact('data'));
    }
    ...
    public function getCardList(Request $request) {
        ...
        $data['breadcrumb1_link'] = route('account.dashboard');
        $data['breadcrumb2'] = 'Payment System';
        $data['breadcrumb2_link'] = route('buyer.paymentsystem');
        $data['breadcrumb3'] = 'Cards';
        $data['result'] = UserCard::where('is_deleted', 0)->where('user_id', auth()->user()->id)->get();

        return view('account.payment.card_list', compact('data'));
    }
    ...
    public function removeCard($card_id) {
        try {
            ...
            if ($cardRes == null) {
                return Redirect::to('/buyer/card-list')->with('error', "Card not valid");
            } else {
                UserCard::where('id', $card_id)->update([
                    'is_deleted' => 1,
                    'updated_at' => date('Y-m-d H:i:s'),
                ]);
                return Redirect::to('/buyer/card-list')->with('success', "Card deleted successfully");
            }
        } catch (\Exception $e) {
            return Redirect::to('/buyer/card-list')->with('error', $e->getMessage());
        }
    }
    ...
    public function addCard(Request $request) {
        ...
        return view('account.payment.add_card', compact('data'));
    }
    ...
    public function saveCard(Request $request) {
        try {
            ...
            if (isset($request->card_id)) {
                ...
                $checkCardExist = UserCard::where('user_id', $user_id)->where('card_number', $request->card_number)
    ...
    public function updateCard($card_id) {
        try {
            ...
            if ($cardRes == null) {
                ...
                return view('account.payment.edit_card', compact('data'));
            }
        } catch (\Exception $e) {
            return Redirect::to('/buyer/card-list')->with('error', $e->getMessage());
        }
