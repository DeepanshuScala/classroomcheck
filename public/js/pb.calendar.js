jQuery(function ($) {
    ...
    var _pb_calendar = (function (target_, options_) {
        ...
        this._target.append('<div class="top-frame">' +
                ...
                '<a class="control-btn prev-btn">' + this._options['prev_month_button'] + '</a>' +
                '<a class="control-btn next-btn">' + this._options['next_month_button'] + '</a>' +
                '</div>' +
                '</div>');
    ...
    _pb_calendar.prototype.update_view = (function () {
        ...
        for (var day_index_ = 0; day_index_ <= day_count_; ++day_index_) {
            ...
            if (this._options['day_selectable']) {
                var clas = '';
                if(parseInt(today) > parseInt(target_moment_.format("YYYYMMDD")) ){
                    clas = 'dis'
                }
                result_html_ += '<a href="#" class="day-label ' + clas +'" data-selectable-day-yyyymmdd="' + target_moment_.format("YYYYMMDD") + '">' + target_moment_.format("DD") + "</a>";
            } else {
        ...
        if (this._options['day_selectable']) {
            this._body_frame.find("[data-selectable-day-yyyymmdd]").click($.proxy(function (event_) {
                ...
                //Deepanshu patch for making selectable
                if(!day_yyyymmdd_.parent().hasClass('before-month')){
                    day_yyyymmdd_.toggleClass('active');
                }
